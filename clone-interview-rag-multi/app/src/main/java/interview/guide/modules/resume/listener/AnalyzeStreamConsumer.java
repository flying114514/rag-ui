...
public class AnalyzeStreamConsumer extends AbstractStreamConsumer<AnalyzeStreamConsumer.AnalyzePayload> {
    ...
    public AnalyzeStreamConsumer(
        ...
        @Value("${app.async.analyze-consumer.workers:2}") int workerCount,
        @Value("${app.async.analyze-consumer.batch-size:10}") int batchSize,
        @Value("${app.async.analyze-consumer.poll-interval-ms:1000}") long pollIntervalMs
    ) {
