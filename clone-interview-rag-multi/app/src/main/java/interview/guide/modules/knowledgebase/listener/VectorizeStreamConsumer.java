...
public class VectorizeStreamConsumer extends AbstractStreamConsumer<VectorizeStreamConsumer.VectorizePayload> {
    ...
    public VectorizeStreamConsumer(
        ...
        @Value("${app.async.vectorize-consumer.workers:2}") int workerCount,
        @Value("${app.async.vectorize-consumer.batch-size:10}") int batchSize,
        @Value("${app.async.vectorize-consumer.poll-interval-ms:1000}") long pollIntervalMs
    ) {
        super(redisService, workerCount, batchSize, pollIntervalMs);
