...
public class EvaluateStreamConsumer extends AbstractStreamConsumer<EvaluateStreamConsumer.EvaluatePayload> {
    ...
    public EvaluateStreamConsumer(
        ...
        @Value("${app.async.evaluate-consumer.workers:2}") int workerCount,
        @Value("${app.async.evaluate-consumer.batch-size:10}") int batchSize,
        @Value("${app.async.evaluate-consumer.poll-interval-ms:1000}") long pollIntervalMs
    ) {
